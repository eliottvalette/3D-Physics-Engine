...
# --- Objets du monde ---
camera = Camera3D()
cube = Cube3D(size=1.0, position=np.array([1.0, 8.0, 1.0], dtype=np.float64))
...
while running:
    ...
    if keys[K_SPACE]:
        cube.position = np.array([1.0, 8.0, 1.0], dtype=np.float64)
        cube.velocity = np.array([0.0, 0.0, 0.0], dtype=np.float64)
        cube.rotation = np.array([0.0, 0.0, 0.0], dtype=np.float64)
        cube.angular_velocity = np.array([0.0, 0.0, 0.0], dtype=np.float64)
